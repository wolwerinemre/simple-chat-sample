...
/* Controllers */
angular.module('simpleChatSample.controllers', ['toaster'])
	.controller('ChatController', ['$scope', '$location', '$interval', 'toaster', 'ChatSocket', function($scope, $location, $interval, toaster, chatSocket) {
		...
		$scope.showHistoryPage = function() {
			var show = $scope.showHistory;
			$scope.showHistory = !show;
			chatSocket.subscribe("/app/chat.history", function(message) {
				$scope.historyMessages = JSON.parse(message.body);
			});
